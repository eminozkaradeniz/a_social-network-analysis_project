...
# html dosyasında etiketler dışındaki tüm metinleri çıkartan fonksiyon
...
with open('../Html-dosyalari/0.html', 'r') as f:
    html = f.read()
...
for i in range(1, len(htmls)):
    ...
    links = dell(links, urls, name=urls[i])
...
# html'lerin içindeki tüm metinler çıkartılıyor.
...
for i in range(1, len(df)):
    with open(f'../Html-dosyalari/{i}.html', mode='w') as f:
        f.write(df['html'][i])
...
df2.to_csv('../Grafikler/gephi/edges.csv', index=False)
df3.to_csv('../Grafikler/gephi/nodes.csv', index=False)
